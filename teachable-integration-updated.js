/**
 * Dictation Checker - Teachable Integration Script (Updated Version)
 * 
 ...
 * </div>
 * 
 * <script src="https://path-to-your-hosted-app/teachable-integration-updated.js"></script>
 */
...
(function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    console.log('Dictation App: Integration script loaded, looking for containers');
    ...
    console.log(`Dictation App: Found ${containers.length} container(s)`);
    ...
    containers.forEach(function(container, index) {
      ...
      iframe.width = '100%';
      iframe.height = '680px'; // Increased default height to show keyboard shortcuts
      ...
      console.log(`Dictation App: Loading iframe with URL: ${iframeUrl}`);
      
      // Clear the container and add the iframe
      container.innerHTML = '';
      container.appendChild(iframe);
      
      // Add responsive height adjustment - handle both message types
      window.addEventListener('message', function(event) {
        if (!event.data || !event.data.type) return;
        
        try {
          // Handle initial ready message
          if (event.data.type === 'DICTATION_APP_READY') {
            if (event.data.height) {
              // Add a small buffer to the height
              const desiredHeight = Math.max(680, event.data.height + 50);
              iframe.height = `${desiredHeight}px`;
              console.log(`Dictation App: Initial resize to ${desiredHeight}px`);
            }
          }
          // Handle dynamic height updates
          else if (event.data.type === 'DICTATION_HEIGHT_UPDATE') {
            if (event.data.height) {
              const newHeight = Math.max(680, event.data.height + 30);
              iframe.height = `${newHeight}px`;
              console.log(`Dictation App: Dynamic resize to ${newHeight}px`);
            }
          }
        } catch (e) {
          console.error('Dictation App: Error processing iframe message', e);
        }
      });
    });
